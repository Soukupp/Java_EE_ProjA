    ...
    <br>
        <el-card  :body-style="{ padding: '0px'}" v-for="(item,index) in Topics" :key="index">
          <div style="padding: 14px;" :key="index"  @click="linkToExpertDetail(item,index)">
            ...
            <Test5 :fatherScore="Topics[index].rating" class="rating"/>
            ...
            <div class="description" >
              ...
              <div class="subdes">
                ...
                <span class="topic" v-for="(item,subIndex) in Topics[index].topic" :key="subIndex"><i style="color:gray; font-size: 20px;">#&nbsp;</i>{{ Topics[index].topic[subIndex].title }}<br></span>  
                </div>
              <br>
              <div>
      ...
      for (var i = 0; i < length; i++)
{
  ...
  mounted() {
    ...
    this.queryData().then(res => {
      ...
      this.getTopics(this.Topics.length);
      this.getCity(this.Topics.length);
    })
