    ...
    <div> 
      ...
      <el-card :body-style="{ padding: '0px'}" v-for="(item,index) in orders" :key="index">
          <div style="padding: 14px;" :key="index">
            ...
            <span>支付时间{{ orders[index].payTime }}</span>
    ...
    this.queryData().then(res => {
      ...
      console.log(res);
        ...
        for (var i = 0; i < length; i++)
        {
            ...
            this.$set(this.orders[i], "second", this.orders[i].payTime[12] + this.orders[i].appointTime[13]);

            this.orders[i].payTime =
                this.orders[i].year + '/' + this.orders[i].month;
